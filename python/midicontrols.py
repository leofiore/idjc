...
class Binding(tuple):
    ...
    MODIFIERS= (
        ...
        (Gdk.EventMask.META_MASK, u'\u25C6'),
        (Gdk.EventMask.SUPER_MASK, u'\u2318'), # win/command
        (Gdk.EventMask.HYPER_MASK, u'\u25CF'),
    )
...
class CustomSpinButton(Gtk.SpinButton):
    def __init__(self, adjustment, climb_rate= 0.0, digits= 0):
        GObject.GObject.__init__(self, adjustment, climb_rate, digits)
    ...
    def on_tooltip_query(self, tv, x, y, kb_mode, tooltip):
        if (x, y) != self.tooltip_coords:
            ...
            if path is not None:
                ...
                hbox = Gtk.HBox()
                hbox.set_spacing(3)
                hbox.pack_start(Gtk.image_new_from_pixbuf(row[3].copy(, True, True, 0)), False)
                hbox.pack_start(Gtk.Label(row[4], True, True, 0), False)
                hbox.pack_start(Gtk.Label("  " + row[5] + row[6], True, True, 0), False)
                if row[7]:
                    hbox.pack_start(Gtk.Label("  " + row[7], True, True, 0), False)
                hbox.show_all()
